...
class DetailWACCModel:
    ...
    def run(self):
        ...
        if not peer_results:
            st.error("유효한 데이터가 없습니다."); return None
...
if btn:
    ...
    if res:
        ...
        with col_str1:
            st.markdown("**A. Target D/E Ratio (목표 부채비율)**")
            st.info(f"Logic: **Peer Group Median** D/E Ratio used.\n\nMedian Value: **{t['DE']:.2%}**")
            st.markdown("- **Why Median?** 개별 기업의 극단값(Outlier)을 배제하고 산업 표준을 반영하기 위함입니다.")
        
        with col_str2:
            st.markdown("**B. Weights Conversion (가중치 계산)**")
            st.latex(r"W_{Equity} = \frac{1}{1 + D/E}, \quad W_{Debt} = \frac{D/E}{1 + D/E}")
            st.write(f"- **Equity Weight:** 1 / (1 + {t['DE']:.2f}) = **{t['We']:.1%}**")
            st.write(f"- **Debt Weight:** {t['DE']:.2f} / (1 + {t['DE']:.2f}) = **{t['Wd']:.1%}**")
        ...
        df = res['peer_df'].copy()
        cols_order = [
            "Ticker", "Company Name", "Currency", "FY Date",
            "Revenue (TTM)", "Revenue (FY)",
            "EBIT (TTM)", "EBIT % (TTM)", "EBIT (FY)", "EBIT % (FY)",
            "EBITDA (TTM)", "EBITDA % (TTM)", "EBITDA (FY)", "EBITDA % (FY)",
            "Levered Beta", "Unlevered Beta", "D/E Ratio", "Market Cap", "Total Debt"
        ]
        ...
        df_disp = df.copy()
        for c in ["Revenue (TTM)", "Revenue (FY)", "EBIT (TTM)", "EBIT (FY)", "EBITDA (TTM)", "EBITDA (FY)", "Market Cap", "Total Debt"]:
            df_disp[c] = df_disp[c].apply(fmt_usd)
        for c in ["EBIT % (TTM)", "EBIT % (FY)", "EBITDA % (TTM)", "EBITDA % (FY)", "D/E Ratio"]:
            df_disp[c] = df_disp[c].apply(fmt_pct)
        for c in ["Levered Beta", "Unlevered Beta"]:
